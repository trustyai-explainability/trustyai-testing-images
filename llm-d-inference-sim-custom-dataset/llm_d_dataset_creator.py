...
SIMPLE_TOKENIZER = re.compile(r'(\s+|,)')
...
def tokenize(text):
    """Tokenize text while preserving spaces between tokens."""
    return SIMPLE_TOKENIZER.split(text)
...
def convert_conversations_to_sqlite(data_folder_path, db_path):
    ...
    session.query(LlmdPrompt).delete()
